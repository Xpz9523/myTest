...
<body>
			...
			<div class="layui-form-item">
				<div class="layui-row">
					...
					<div class="layui-input-block">
						<input type="text" name="sizeName" lay-verify="required" placeholder="" autocomplete="off" class="layui-input" id="sizeName">
					</div>
				</div>
			</div>
    ...
    <div class="layui-input-block">
      <textarea name="sizeRemark" class="layui-textarea"  id="sizeRemark"></textarea>
    </div>
  </div>
	...
	function listUsers(curr) {
					...
					$.ajax({
					'method' : 'get',
					'url' : ' ${pageContext.request.contextPath}/admin/Big/selectAllSize',
					'dataType' : 'json',
					data : {
						"pageNum" : curr || 1
					},
					'success' : function(page) {
						var html_content = "";
						$.each(page.list,function(i,size) {
											html_content += "<tr>";
											html_content += "<td>"+ (i + 1)+ "</td>";
											html_content += "<td>"+ size.sizeName+ "</td>";
											/*html_content += "<td>"+ document.write(GoodTypeBig.typeBigRemark.substring())+ "</td>";*/
											html_content += "<td>"+ size.sizeRemark+ "</td>";
											html_content += "<td>"+size.createTime + "</td>";
											html_content += "<td>"+ size.updateTime+ "</td>";
											...
											html_content += "<td><a href='${webpath}/admin/Big/updateSize?pkSizeId="+size.pkSizeId+" ' class='layui-btn layui-btn-norma' >编辑</a><a href='#' class='layui-btn layui-btn-warm'  onclick='deleteBigGoodType("+size.pkSizeId+")' >删除</a></td>";
											html_content += "</tr>";
											});
	...
	function addBigGoodTyep(){
		...
		$("#saveBtn").click(function(){
						...
						var sizeRemark = $("#sizeRemark").val	();
						$.ajax({
							'url' :'${pageContext.request.contextPath}/admin/Big/addsize',
							'type':'post',
							'dataType':'json',
							data:{'sizeName' :sizeName,'sizeRemark':sizeRemark},
		...
		function deleteBigGoodType(id){
			layui.use([ 'form', 'layer', 'laypage', 'laydate' ],
				...
				if(window.confirm('你确定删除吗？')){
					$.ajax({
						url:'${webPath}/admin/Big/deleteSize',
						type:'post',
						datatype:'json',
						data:{
							"bid":id,
