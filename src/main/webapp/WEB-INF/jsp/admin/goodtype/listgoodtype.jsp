...
<body>
	...
	<form class="layui-form" action="" style="display: none;margin-top: 10px;" id="addForm">
			...
			<div class="layui-form-item layui-col-md10">
				<div class="layui-row">
					<label class="layui-form-label">大类型名称</label>
					<div class="layui-input-block">
						<input type="text" name="typeBigName" lay-verify="required" placeholder="大类型名称" autocomplete="off" class="layui-input" id="typeBigName">
					</div>
				</div>
			</div>
    ...
    <div class="layui-input-block">
      <textarea placeholder="" name="typeBigRemark" class="layui-textarea"  id="typeBigRemark"></textarea>
    </div>
  </div>
		...
		<!-- 	<div class="layui-form-item layui-col-md10">
				<div class="layui-row">
					<div class="layui-col-md-offset4 layui-col-md6">
						<button class="layui-btn" id="saveBtn">确认</button>
					</div>
	...
	function listUsers(curr) {
					...
					$.ajax({
					'method' : 'get',
					'url' : ' ${pageContext.request.contextPath}/admin/Big/selectAllGoodTypes',
					'dataType' : 'json',
					data : {
						"pageNum" : curr || 1
					},
					'success' : function(page) {
						var html_content = "";
						$.each(page.list,function(i,GoodTypeBig) {
											html_content += "<tr>";
											html_content += "<td>"+ (i + 1)+ "</td>";
											html_content += "<td>"+ GoodTypeBig.typeBigName+ "</td>";
											/*html_content += "<td>"+ document.write(GoodTypeBig.typeBigRemark.substring())+ "</td>";*/
											html_content += "<td>"+GoodTypeBig.typeBigRemark.substring(0,50)+ "...."+"</td>";
											html_content += "<td>"+GoodTypeBig.createTime + "</td>";
											html_content += "<td>"+ GoodTypeBig.updateTime+ "</td>";
											...
											html_content += "<td><a href='${webpath}/admin/Big/updateBigType?pkTypeBigId="+GoodTypeBig.pkTypeBigId+" ' class='layui-btn layui-btn-norma' >编辑</a><a href='#' class='layui-btn layui-btn-warm'  onclick='deleteBigGoodType("+GoodTypeBig.pkTypeBigId+")' >删除</a></td>";
											html_content += "</tr>";
											});
	...
	function addBigGoodTyep(){
		...
		$("#saveBtn").click(function(){
				 		var typeBigRemark = $("#typeBigRemark").val();
						var typeBigName = $("#typeBigName").val	();
						$.ajax({
							'url' :'${pageContext.request.contextPath}/admin/Big/addBigGoodTypes',
							'type':'post',
							'dataType':'json',
							data:{'typeBigRemark' :typeBigRemark,'typeBigName':typeBigName},
		...
		function deleteBigGoodType(id){
			layui.use([ 'form', 'layer', 'laypage', 'laydate' ],
				...
				if(window.confirm('你确定删除吗？')){
					$.ajax({
						url:'${webPath}/admin/Big/deleteBigGoodType',
						type:'post',
						datatype:'json',
						data:{
							"bid":id,
